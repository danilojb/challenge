...
func main() {
	...
	var sqsSvc *sqs.SQS
	if sqsQueueURL != "" {
		// Configure endpoint to point to localstack if queue URL seems local
		cfg := &aws.Config{Region: aws.String(awsRegion)}
		// derive base endpoint (scheme://host:port) from sqsQueueURL
		// simple heuristic: strip path after service/queue
		if strings.HasPrefix(sqsQueueURL, "http") {
			if idx := strings.Index(sqsQueueURL, "/"); idx != -1 {
				// keep scheme://host:port
				parts := strings.SplitN(sqsQueueURL, "/", 4)
				cfg.Endpoint = aws.String(parts[0] + "//" + parts[2])
			}
		}
