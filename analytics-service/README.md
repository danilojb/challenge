...
> 🛠 **Usando LocalStack?**
> Caso esteja rodando o conjunto de microserviços com Docker/LocalStack, você pode injetar variáveis de ambiente dummy:
> ```bash
...
> ```
> O código agora detecta automaticamente o endpoint base e cria os clientes Boto3 usando-o, evitando erros de token inválido.
...
> ✅ **Nota:** dentro do container a aplicação escuta na porta **8000**; o mapeamento `8005:8000` do `docker-compose.yml` expõe essa porta no host. Fora do Docker ajuste o `PORT` conforme preferir.
> ✅ **Nota:** o serviço dentro do container escuta na porta **8000** (veja `ENV PORT`); o mapeamento `8005:8000` em `docker-compose.yml` expõe essa porta no host. Fora do Docker ajuste o `PORT` conforme necessário.
