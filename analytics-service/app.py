...
try:
    ...
    # Determine endpoint base from SQS URL (LocalStack)
    endpoint_url = None
    if SQS_QUEUE_URL.startswith("http"):
        parts = SQS_QUEUE_URL.split("/", 3)
        if len(parts) >= 3:
            endpoint_url = parts[0] + "//" + parts[2]
    kwargs = {}
    if endpoint_url:
        kwargs["endpoint_url"] = endpoint_url
        log.info(f"Forçando endpoint AWS para {endpoint_url}")
    sqs_client = session.client("sqs", **kwargs)
    dynamodb_client = session.client("dynamodb", **kwargs)
...
if __name__ == '__main__':
    # default to container port 8000 to match docker-compose mapping
